    ...
    internal static class ObjectExtensions
    {
        ...
        private static void HandleTyped(object source, object destination)
        {
            foreach (var srcProp in GetProperties(source))
            {
                ...
                {
                    //HandleTypedEnumerable(source, destination, srcProp, targetProperty, true);
                    continue;
                }
                ...
                if (srcProp.PropertyType != null && IsPropertyReferenceType(srcProp) && IsPropertyReferenceType(targetProperty))
                {
                ...
                //if (srcProp.PropertyType != null && srcProp.PropertyType == typeof(Int64))
                    //srcProp.PropertyType = typeof(Int32);

                //if (srcProp.PropertyType != null && !targetProperty.PropertyType.IsAssignableFrom((Type)srcProp.PropertyType))
                    //continue;

                var val = GetValue(source,                        srcProp);
                targetProperty.SetValue(destination, val, null);
            }
        }

        private static void HandleTypedEnumerable(object source, object destination, dynamic srcProp, PropertyInfo targetProperty, bool nul = false)
        {
            if (nul)
            {
        ...
        private static void HandleExpando(object source, object destination)
        {
            foreach (var srcProp in GetProperties(source))
            {
                ...
                else if (IsPropertyReferenceType(srcProp))
                {
        ...
        {
            if (srcProp.PropertyType == typeof(Int64))
                return Convert.ToInt32(source is ExpandoObject ? srcProp.Value : srcProp.GetValue(source, null));
